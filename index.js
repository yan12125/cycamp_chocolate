...
$(document).on('ready', function(e){
    ...
    $.ajax({
        ...
        'success': function(_data, status, xhr){
            ...
            for(var i in stands)
            {
                $('#page4 #pay_location').append('<input type="radio" name="stand" value="'+i+'" id="stand_'+i+'"><label for="stand_'+i+'">'+stands[i]+'</label><br>');
            }
            ...
            $('#page1 #add_card').on('change', function(e){
                data.add_card = ($('#page1 #add_card').attr('checked')==='checked')?1:0;
            });
